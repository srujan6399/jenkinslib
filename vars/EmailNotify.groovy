...
def call(body) {
  ...
  def attachment = config.attachment
    ...
    String body="<p>This Email is for Demo purpose only -${status}: Job'[${env.JOB_NAME}][${env.BUILD_NUMBER}]':</p>"+
                "<p><b>Check console output at <a href='${env.BUILD_URL}'>${env.JOB_NAME}[${env.BUILD_NUMBER}]</a></b></p>";
     
    emailext body: body, subject: sub, to:tolist, mimeType: 'text/html', attachmentsPattern: attachment
